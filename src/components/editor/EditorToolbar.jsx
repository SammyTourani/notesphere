...
const EditorToolbar = ({ editor }) => {
  ...
  useEffect(() => {
    ...
    const hasAnimated = sessionStorage.getItem('toolbarAnimated') === 'true';
    ...
    if (hasAnimated) {
      ...
      sessionStorage.setItem('toolbarAnimated', 'true');
    }
    
    // Clean up the animation flag when component unmounts
    return () => {
      // We intentionally DON'T clean up sessionStorage to remember across remounts
    };
  }, []);
  ...
  const itemVariants = {
    ...
    visible: (custom) => ({
      ...
      scale: 1,
      transition: {
        delay: custom * 0.05, // Staggered delay based on item index
        duration: 0.3,
  ...
  // Create button elements with proper animation
  const buttons = [
    // Bold button
    ...
    <motion.div key="headings" custom={4} variants={itemVariants} className="relative">
      ...
      <AnimatePresence>
        {activeDropdown === 'heading' && (
          ...
          >
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('heading', { level: 1 }) ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
            ...
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('heading', { level: 2 }) ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
                editor.chain().focus().toggleHeading({ level: 2 }).run();
                setActiveDropdown(null);
              }}
            >
              <span className="text-md font-semibold">Heading 2</span>
            </button>
            
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('heading', { level: 3 }) ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
            ...
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('paragraph') ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
    ...
    <motion.div key="lists" custom={6} variants={itemVariants} className="relative">
      ...
      <AnimatePresence>
        {activeDropdown === 'list' && (
          ...
          >
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('bulletList') ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
            ...
            <button 
              className={`toolbar-dropdown-item ${editor.isActive('orderedList') ? 'text-indigo-600 dark:text-indigo-400' : ''}`}
              onClick={() => {
    ...
    // Unlink button - conditionally rendered
    editor.isActive('link') && (
      <motion.div key="unlink" custom={9} variants={itemVariants}>
        <EditorMenuButton
          onClick={() => editor.chain().focus().unsetLink().run()}
          tooltip="Remove Link"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M18.364 18.364A9 9 0 1 1 5.636 5.636a9 9 0 0 1 12.728 12.728z"></path>
    ...
    // Separator (only show if link is active)
    editor.isActive('link') && (
      <motion.div key="sep-4" custom={10} variants={itemVariants} className="toolbar-separator"></motion.div>
    ),
